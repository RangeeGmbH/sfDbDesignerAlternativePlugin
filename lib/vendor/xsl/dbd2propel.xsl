...
FOREIGN-KEY: foreignTable, name, onDelete, onUpdate
...
Christoph Schaefer - <christoph.schaefer <<at>> sinntax.de> - onUpdate support, Bug fixes
...
version 0.5 (2009-04-09) - onUpdate support, Bug fixes
...
<xsl:template name="show_ForeignKey">
		...
		<xsl:attribute name="onDelete">
			...
			<xsl:variable name="actionId">
				<xsl:call-template name="str_replace">
					<xsl:with-param name="stringIn" select="substring-before(substring-after($relation/@RefDef,'OnDelete'), '\n')"/>
    ...
    <!-- onUpdate -->
		<xsl:attribute name="onUpdate">
			...
			<xsl:variable name="actionId">
				<xsl:call-template name="str_replace">
					<xsl:with-param name="stringIn" select="substring-before(substring-after($relation/@RefDef,'OnUpdate'), '\n')"/>
					<xsl:with-param name="charsIn" select="'='"/>
					<xsl:with-param name="charsOut" select="''"/>
				</xsl:call-template>
			</xsl:variable>
